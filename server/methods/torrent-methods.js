...
Meteor.methods({
  ...
  */
  ...
  /**
   * Fix WebTorrent extended protocol handshake issues
   * This addresses the "Unrecognized extension: handshake" error
   */
  'torrents.fixExtendedProtocol': function(infoHash) {
    check(infoHash, String);
    
    console.log(`🔧 FIXING EXTENDED PROTOCOL for torrent ${infoHash}`);
    ...
    try {
      ...
      result.actions.push(`🔧 Fixing extended protocol for: ${torrent.name}`);
      result.actions.push(`📊 Status: files=${torrent.files?.length || 0}, peers=${torrent.numPeers}, ready=${torrent.ready}`);
      
      // Fix existing wire connections with proper protocol
      if (torrent.wires && torrent.wires.length > 0) {
        result.actions.push(`🔌 Fixing ${torrent.wires.length} wire connections with proper protocol`);
        
        torrent.wires.forEach(function(wire, index) {
          try {
            result.actions.push(`🔌 Wire ${index}: ${wire.remoteAddress}:${wire.remotePort}`);
            
            // Check what extensions the wire actually supports
            const wireExtensions = wire.peerExtensions ? Object.keys(wire.peerExtensions) : [];
            const supportsExtended = wireExtensions.includes('extended');
            
            result.actions.push(`   Extensions: [${wireExtensions.join(', ')}], extended: ${supportsExtended}`);
            
            if (!supportsExtended) {
              result.actions.push(`   ❌ Peer doesn't support extended protocol - cannot fix`);
              return;
            }
            ...
            // Load ut_metadata extension properly
            try {
              const ut_metadata = require('ut_metadata');
              
              // For seeding torrents, provide metadata
              if (torrent.ready && torrent.metadata) {
                wire.use(ut_metadata(torrent.metadata));
                result.actions.push(`   ✅ Installed ut_metadata with ${torrent.metadata.length} bytes for seeding`);
              } else {
              ...
              // Wait for the extension to be properly initialized
              setTimeout(function() {
                // Check if ut_metadata is now available
                if (wire.ut_metadata) {
                  result.actions.push(`   ✅ ut_metadata extension confirmed active`);
                  
                  // For downloading torrents, try to fetch metadata
                  if (!torrent.ready && typeof wire.ut_metadata.fetch === 'function') {
                    try {
                      wire.ut_metadata.fetch();
                      result.actions.push(`   📥 Requested metadata via ut_metadata.fetch()`);
                    } catch (fetchErr) {
                      result.actions.push(`   ⚠️ Fetch error: ${fetchErr.message}`);
                    }
                  }
                } else {
                  result.actions.push(`   ❌ ut_metadata extension not available after installation`);
                }
              }, 1000);
              
            } catch (extensionErr) {
              result.actions.push(`   ❌ Extension installation error: ${extensionErr.message}`);
            }
            
            // Send proper interest and unchoke signals
            try {
              if (!wire.amInterested) {
                wire.interested();
              ...
              result.actions.push(`   ⚠️ Signaling error: ${signalErr.message}`);
            }
      ...
      // Set up proper wire handler for new connections
      result.actions.push(`🔧 Setting up proper protocol handler for new connections`);
      
      const originalOnWire = torrent._onWire?.bind(torrent);
      torrent._onWire = function(wire) {
        result.actions.push(`🔌 New connection: ${wire.remoteAddress} - applying protocol fix`);
        ...
        // Apply our protocol fix after wire is initialized
        setTimeout(function() {
          try {
            const wireExtensions = wire.peerExtensions ? Object.keys(wire.peerExtensions) : [];
            const supportsExtended = wireExtensions.includes('extended');
            
            if (supportsExtended && !wire.ut_metadata) {
              const ut_metadata = require('ut_metadata');
              
              if (torrent.ready && torrent.metadata) {
                wire.use(ut_metadata(torrent.metadata));
                result.actions.push(`✅ Auto-installed seeding metadata for ${wire.remoteAddress}`);
              } else {
                wire.use(ut_metadata());
                result.actions.push(`✅ Auto-installed downloading metadata for ${wire.remoteAddress}`);
              }
            }
          } catch (autoErr) {
            result.actions.push(`⚠️ Auto-install error: ${autoErr.message}`);
          }
        }, 1500); // Longer delay to ensure wire is ready
      };
      
      result.success = true;
      result.actions.push(`🎉 Extended protocol fix applied successfully`);
      
      return result;
      
    } catch (error) {
      console.error('Extended protocol fix error:', error);
      result.error = error.message;
      result.actions.push(`❌ Error: ${error.message}`);
      return result;
    }
  ...
  /**
   * Alternative approach: Force metadata exchange without extended handshake
   */
  'torrents.alternativeMetadataFix': async function(infoHash) {
    check(infoHash, String);
    
    console.log(`🔄 ALTERNATIVE METADATA FIX for torrent ${infoHash}`);
    ...
    try {
      ...
      if (!torrent) {
        ...
        result.actions.push('🔄 Reloading torrent for alternative fix');
        torrent = await WebTorrentServer.addTorrent(torrentRecord.magnetURI);
      }
      
      result.actions.push(`📊 Initial: files=${torrent.files?.length || 0}, peers=${torrent.numPeers}, ready=${torrent.ready}`);
      
      // Strategy: Use WebTorrent's built-in extension loading without custom handshakes
      if (torrent.wires && torrent.wires.length > 0) {
        result.actions.push(`🔧 Applying alternative fix to ${torrent.wires.length} connections`);
        
        for (const wire of torrent.wires) {
          try {
            result.actions.push(`🔌 Wire: ${wire.remoteAddress}`);
            
            // Let WebTorrent handle the protocol naturally
            if (wire.peerExtensions && wire.peerExtensions.extended) {
              result.actions.push(`   ✅ Peer supports extended protocol`);
              
              // If peer supports ut_metadata, let it be handled by WebTorrent
              if (wire.peerExtensions.ut_metadata) {
                result.actions.push(`   ✅ Peer already supports ut_metadata`);
                
                // For downloading, simply request
                if (!torrent.ready && wire.ut_metadata && wire.ut_metadata.fetch) {
                  wire.ut_metadata.fetch();
                  result.actions.push(`   📥 Requested metadata using existing extension`);
                }
              } else {
                result.actions.push(`   ⚠️ Peer doesn't support ut_metadata`);
                
                // Try to load the extension without custom handshake
                try {
                  const ut_metadata = require('ut_metadata');
                  
                  // Just use the extension, let WebTorrent handle handshake
                  if (torrent.metadata) {
                    wire.use(ut_metadata(torrent.metadata));
                  } else {
                    wire.use(ut_metadata());
                  ...
                  result.actions.push(`   ✅ Loaded ut_metadata extension (WebTorrent will handle handshake)`);
                  
                } catch (loadErr) {
                  result.actions.push(`   ❌ Extension load error: ${loadErr.message}`);
                }
              }
              
              // Send basic peer protocol signals
              if (!wire.amInterested) {
                wire.interested();
                result.actions.push(`   📢 Sent interested`);
              }
              ...
              result.actions.push(`   ❌ Peer doesn't support extended protocol`);
            }
      ...
      // Simple announce without DHT complications
      result.actions.push(`📢 Simple tracker announce`);
      try {
        if (typeof torrent.announce === 'function') {
          torrent.announce();
      ...
      // Wait for results
      result.actions.push(`⏳ Waiting for metadata exchange (45 seconds)`);
      ...
      return new Promise(function(resolve) {
        const checker = Meteor.setInterval(function() {
          ...
          if (elapsed % 10000 < 2000) {
            result.actions.push(`📊 ${elapsed}ms: ready=${status.ready}, files=${status.files}, peers=${status.peers}, progress=${status.progress}%`);
          }
          ...
          if (torrent.ready && torrent.files && torrent.files.length > 0) {
            ...
            result.success = true;
            result.finalStatus = status;
            result.actions.push(`🎉 ALTERNATIVE SUCCESS after ${elapsed}ms!`);
      ...
      console.error('Alternative metadata fix error:', error);
      result.error = error.message;
      result.actions.push(`❌ Error: ${error.message}`);
      return result;
    }
  ...
  /**
   * Core protocol debugging - show what's actually happening
   */
  'torrents.debugExtendedProtocol': function(infoHash) {
    check(infoHash, String);
    
    console.log(`🔍 DEBUGGING EXTENDED PROTOCOL for torrent ${infoHash}`);
    ...
    try {
      ...
      if (!torrent) {
        result.diagnosis.error = 'Torrent not found in client';
        return result;
      }
      ...
      if (torrent.wires && torrent.wires.length > 0) {
        torrent.wires.forEach(function(wire, index) {
          ...
          const wireInfo = {
            ...
            supportsExtended: !!(wire.peerExtensions && wire.peerExtensions.extended),
          ...
          // Try to get more details about the extension state
          if (wire.ut_metadata) {
            wireInfo.utMetadataDetails = {
              id: wire.ut_metadata.id,
              metadata: !!wire.ut_metadata.metadata,
              hasFetch: typeof wire.ut_metadata.fetch === 'function'
            };
          }
          
          result.wireDetails.push(wireInfo);
        });
      }
      
      console.log('Extended protocol debug result:', result);
      return result;
      
    } catch (error) {
      console.error('Extended protocol debug error:', error);
